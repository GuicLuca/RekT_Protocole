...
use crate::libs::utils::{
    diff_hashsets, get_bytes_from_slice, get_u16_at_pos, get_u32_at_pos, get_u64_at_pos,
    u8_to_vec_be, vec_to_u8,
};
...
fn test_diff_hashset() {
    ...
    assert_eq!(
        (vec!(6 as TopicId), vec!(1 as TopicId)),
        diff_hashsets(&new_set, &original_set)
    );
}
